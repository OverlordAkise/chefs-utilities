--Chef's BlockedPropList
--Module of chefs-utilities
--Made by OverlordAkise and Crumble

chef.plist = {}

hook.Add("PlayerSpawnProp","chef_disableProps",function(ply, model)
  local res = sql.Query("SELECT * FROM FPP_BLOCKEDMODELS1 WHERE model = 'models/chef.mdl'")
  if res == nil then
    local stime = SysTime()
    print("[chef] Adding blockedmodels...")
    sql.Begin()
    for k,v in pairs(chef.plist) do
      sql.Query("INSERT INTO FPP_BLOCKEDMODELS1(model) VALUES('"..v.."')")
    end
    sql.Query("INSERT INTO FPP_BLOCKEDMODELS1(model) VALUES('models/chef.mdl')")
    sql.Commit()
    print("[chef] Finished blockedmodels!")
    print("[chef] Time: "..(SysTime()-stime).."s")
  else
    print("[chef] Blockedmodels already added!")
  end
end)
